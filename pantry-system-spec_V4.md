...
### iPad Software Strategy

Three escalation levels depending on what limitations we hit:

```
┌─────────────────────────────────────────────────────────────────┐
│                    iPAD SOFTWARE OPTIONS                         │
│                                                                  │
│  PLAN A: Stock iOS + Safari (Start Here)                        │
│  ────────────────────────────────────────                       │
│  • Safari → http://[PI_IP]:8000                                 │
│  • "Add to Home Screen" for app-like experience                 │
│  • Guided Access for kiosk mode                                 │
│  • Effort: Minimal                                              │
│  • Limitations: Safari chrome visible, some JS restrictions     │
│                                                                  │
│                          ↓ If limitations are annoying          │
│                                                                  │
│  PLAN B: Stock iOS + WebView Wrapper App                        │
│  ───────────────────────────────────────                        │
│  • Simple native app (~50 lines Objective-C)                    │
│  • Full-screen, no Safari UI                                    │
│  • Same web content from Pi                                     │
│  • Effort: Low (1-2 hours)                                      │
│  • Limitations: Signing (7-day free cert or $99/yr dev account) │
│                                                                  │
│                          ↓ If signing is annoying               │
│                                                                  │
│  PLAN C: Jailbreak + Permanent Install                          │
│  ─────────────────────────────────────                          │
│  • Checkra1n jailbreak (works on iPad 4 / A6X)                  │
│  • AppSync Unified allows unsigned apps                         │
│  • True kiosk mode (better than Guided Access)                  │
│  • Effort: Medium (1-2 hours first time)                        │
│  • Limitations: Semi-tethered (re-jailbreak after reboot)       │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### Plan A: Safari (Default)

Just point Safari at the Pi and use iOS's built-in features:

**Setup:**
1. Open Safari → `http://192.168.1.XX:8000`
2. Tap Share → "Add to Home Screen"
3. Settings → Accessibility → Guided Access → Enable
4. Open the home screen icon, triple-click home button to lock

**Pros:** Zero effort, no tools needed
**Cons:** Safari UI visible, Guided Access can be exited with passcode

### Plan B: WebView Wrapper App

A minimal native iOS app that's just a full-screen web view:
...
- (void)viewDidLoad {
    ...
    // Point to Pi's local IP - update this to match your network
    NSURL *url = [NSURL URLWithString:@"http://192.168.1.50:8000"];
    [self.webView loadRequest:[NSURLRequest requestWithURL:url]];
}

- (BOOL)prefersStatusBarHidden {
    return YES;  // Full screen, no status bar
}
...
**Requirements:**
- Mac with Xcode 12 (last version supporting iOS 10 deployment)
- Apple ID (free or paid)
- USB cable to iPad

**Signing options:**

| Option | Cost | Duration | Re-sign Process |
|--------|------|----------|-----------------|
| Free Apple ID | $0 | 7 days | Connect to Mac, rebuild in Xcode |
| Developer Account | $99/yr | 1 year | Rebuild annually |
| Jailbreak (Plan C) | $0 | Permanent | None needed |

### Plan C: Jailbreak

The iPad 4 has an A6X chip vulnerable to the checkm8 bootrom exploit, making it permanently jailbreakable. This solves the app signing problem and enables true kiosk mode.

**What jailbreaking enables:**

| Feature | Stock iOS | Jailbroken |
|---------|-----------|------------|
| Install unsigned apps | ✗ (7-day limit) | ✓ Permanent |
| True kiosk lockdown | ⚠️ Guided Access | ✓ Full control |
...
```
CHECKRA1N JAILBREAK (iPad 4 / iOS 10.3.3)
─────────────────────────────────────────

Requirements:
• Mac or Linux computer
• USB cable
• checkra1n tool (https://checkra.in)

Steps:
1. Download checkra1n for your platform
2. Connect iPad via USB
3. Run checkra1n
4. Follow prompts to enter DFU mode:
   - Hold Power + Home for 10 seconds
   - Release Power, keep holding Home for 10 more seconds
5. Wait for jailbreak to complete (~2 minutes)
6. iPad reboots with checkra1n loader app installed
7. Open checkra1n app on iPad → Install Cydia

Post-jailbreak setup:
1. Open Cydia (package manager)
2. Add repo: https://cydia.akemi.ai/
3. Install "AppSync Unified" (allows unsigned apps)
4. Install your WebView app via Xcode (now permanent)
5. Optional: Install kiosk tweaks (see below)
```

**Recommended Cydia tweaks for kiosk mode:**

| Tweak | Purpose |
|-------|---------|
| AppSync Unified | Install unsigned apps permanently |
| Activator | Custom button/gesture actions |
| NoLockScreen | Skip lock screen entirely |
| SBPowerAlert | Disable "slide to power off" |
| Mikoto | Disable control center, notification center |
| FullSafari | Remove Safari UI (if using Safari instead of WebView) |

**Important notes:**
- **Semi-tethered:** After a reboot, the iPad works normally but jailbreak features are disabled until you re-run checkra1n from a computer. For a wall-mounted kiosk that rarely reboots, this is fine.
- **Reversible:** Restoring iPad via iTunes removes the jailbreak completely.
- **Stable:** checkm8/checkra1n is extremely stable on A6X devices; this isn't a risky hack.

**Kiosk lockdown example (with Activator):**

```
Activator Rules:
• Home button single press → Do nothing
• Home button double press → Do nothing  
• Home button hold → Do nothing
• Power button → Do nothing (or trigger specific action)
• Swipe from bottom → Disabled
• Swipe from top → Disabled

Result: User can ONLY interact with the PantryPal touch UI
```

### iOS 10 Safari Compatibility

Regardless of which plan you use, the frontend must work within iOS 10 Safari's limitations:
